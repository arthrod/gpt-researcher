...
class ScraperRepository:
    ...
    async def _get_embedding_async(self, content: str) -> list[float] | None:
        ...
        try:
            ...
            embeddings_list = await loop.run_in_executor(
                self._executor, self.embeddings.embed_documents, [content]
            )
    ...
    async def save_records(
        self, records: list[dict[str, Any]], batch_size: int = 50, upsert: bool = True
    ) -> int:
        ...
        async with self.get_session() as session:
            ...
            for i in range(0, len(records), batch_size):
                ...
                embedding_tasks = [
                    self._get_embedding_async(record["raw_content"]) for record in batch
                ]
                ...
                if upsert:
                    ...
                    for doc_data in documents:
                        try:
                            ...
                            logger.debug(
                                f"Skipping duplicate URL {doc_data['url']}: {e}"
                            )
    ...
    async def get_document_by_url(self, url: str) -> ScrapedDocument | None:
    ...
    async def get_documents_batch(self, urls: list[str]) -> list[ScrapedDocument]:
    ...
    async def update_embeddings(self, urls: list[str], batch_size: int = 20) -> int:
        ...
        for i in range(0, len(urls), batch_size):
            ...
            async with self.get_session() as session:
                ...
                embedding_tasks = [
                    self._get_embedding_async(doc.content) for doc in documents
                ]
                embeddings = await asyncio.gather(*embedding_tasks)
    ...
    async def get_statistics(self) -> dict[str, Any]:
        ...
        async with self.get_session() as session:
            ...
            embedding_stmt = (
                select(func.count())
                .select_from(ScrapedDocument)
                .where(ScrapedDocument.embedding.isnot(None))
            )
