...
async def save_scraped_data_optimized(
    ...
    repository = ScraperRepository(
        db_url=db_url, embeddings=embeddings, pool_size=20, max_overflow=10
    )
    ...
    try:
        ...
        saved_count = await repository.save_records(
            records=records, batch_size=batch_size, upsert=upsert
        )
...
class ScraperDataManager:
    ...
    def __init__(
        self, db_url: str, embeddings: Any | None = None, pool_size: int = 20
    ):
        ...
        self.repository = ScraperRepository(
            db_url=db_url, embeddings=embeddings, pool_size=pool_size
        )
    ...
    async def save_batch(
        self, records: list[dict[str, Any]], batch_size: int = 50
    ) -> int:
    ...
    async def update_embeddings_for_urls(
        self, urls: list[str], batch_size: int = 20
    ) -> int:
        ...
        """
        return await self.repository.update_embeddings(urls=urls, batch_size=batch_size)
