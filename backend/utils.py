...
async def write_to_file(filename: str, text: str) -> None:
    ...
    if not isinstance(text, str):
        logger.warning(f"Non-string input provided (type: {type(text).__name__}), converting to string")
        text = str(text)
...
async def write_md_to_pdf(text: str, filename: str = "") -> str:
    ...
    try:
        ...
        if len(text) > 1000000:  # 1MB limit
            logger.warning(f"Text is very large ({len(text)} chars), truncating for PDF conversion")
        ...
        logger.error(f"Check write permissions for directory: {os.path.dirname(file_path)}")
        return ""
    except Exception as e:
...
async def write_md_to_word(text: str, filename: str = "") -> str:
    ...
    try:
        ...
        # Validate markdown content before processing
        if len(text) > 1000000:  # 1MB limit
            logger.warning(f"Text is very large ({len(text)} chars), truncating for DOCX conversion")
            text = text[:1000000] + "\n\n[Content truncated due to size limits]"
